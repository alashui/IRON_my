...
int main(int argc, char **argv)
{
    ...
    //NDTLiteConfig ndtcfg2;
    // some important variables
    ndtcfg.cellSize = 0.1f;          // default: 0.1m x 0.1m x 0.1m cells
    ndtcfg.subsamplingFactor = 0.5f; // default: take only 20% of points (drawn randomly)
    ...
    //NDTMapLiteCreator<float, std::vector<Pt3D>, Pt3DAccessor> creator2(ndtcfg);

    // prepare IRON engine (IRON.h for a summary of available options)
    IRONConfig ironcfg;
    // important variables
    ironcfg.matchingTolerance = 0.1f;   // RANSAC inlier threshold: higher values will make registration more robust
                                           // but the transform computation more inaccurate; default: half of NDT-cell size
    ironcfg.entropyThreshold = 0.05f;    // the lower, the more keypoints will be found and the slower registration
    ironcfg.neighborSearchRadius = 1.5f; // radius around each NDT-cell for searching neighboring cells
    ...
    std::cerr << "NDT MAP 2..: " << ndtMap2.cells.size() << " cells\n";
    std::cerr << "KEYPOINTS 2: " << descriptors2.size() << " cells\n";
